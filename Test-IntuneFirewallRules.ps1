...
try {
    ...
    # Perform the API request with the Microsoft Graph PowerShell SDK
    $response = (Invoke-MgGraphRequest -Uri $uri -Method GET).value

    # Output response (you can process it further as needed)
    Write-Host "Request successful. Data retrieved:" -ForegroundColor Green
    ...
    # Attempt to access the response content if available
    if ($ex.Response -and $ex.Response.Content) {
        ...
        Write-Error "Request failed, but no error response was returned."
    }

    # Optional: Stop execution or handle further as needed
    break
}
...
Function Export-JSONData {
  ...
  try {
      ...
      if (-not (Test-Path -Path $ExportPath)) {
          Write-Host "Creating temporary folder $ExportPath" -ForegroundColor Yellow
          try {
              New-Item -ItemType Directory -Path $ExportPath -Force | Out-Null
              Write-Host "Directory created at $ExportPath" -ForegroundColor Green
          } catch {
              Write-Host "Failed to create directory at $ExportPath. Please check permissions." -ForegroundColor Red
              return
          }
      }
      ...
      if ([string]::IsNullOrEmpty($JSON)) {
          Write-Host "No JSON specified, please provide valid JSON..." -ForegroundColor Red
          return
      }

      # Convert JSON for processing
      $JSON1 = ConvertTo-Json $JSON -Depth 5
      $JSON_Convert = $JSON1 | ConvertFrom-Json

      # Generate a filename
      $DisplayName = $JSON_Convert.displayName -replace '[<>:"/\\|?*]', "_"
      $FileName_JSON = "$DisplayName" + "_" + (Get-Date -Format "dd-MM-yyyy-H-mm-ss.fff") + ".json"

      # Write JSON content to file
      Write-Host "Exporting JSON to $ExportPath\$FileName_JSON" -ForegroundColor Cyan
      $JSON1 | Set-Content -LiteralPath "$ExportPath\$FileName_JSON"
      Write-Host "JSON created successfully at $ExportPath\$FileName_JSON" -ForegroundColor Green
  } catch {
...
####################################################
        ...
        $RuleJSON = Get-ChildItem $xportpath\*.json
